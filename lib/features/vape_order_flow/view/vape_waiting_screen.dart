...
class VapeWaitingScreen extends ConsumerStatefulWidget {
  const VapeWaitingScreen({super.key, this.orderIds = const []});

  final List<String> orderIds;

  @override
  ConsumerState<VapeWaitingScreen> createState() => _VapeWaitingScreenState();
}

class _VapeWaitingScreenState extends ConsumerState<VapeWaitingScreen> {
  ...
  Future<void> _poll() async {
    ...
    if (allAccepted) {
      ...
      _tickTimer?.cancel();
      if (needsPay) {
        context.pushReplacement(VapeOrderFlowRoutes.payFor(ids));
      } else {
        context.pushReplacement(VapeOrderFlowRoutes.confirmedFor(ids));
      }
    }
  }
      ...
      body: Padding(
        ...
        child: Column(
          children: [
            ...
            SizedBox(height: 16.h),
            VapeOrderSummaryRow(summary: _summary, total: _total),
            const Spacer(),
            _CancelOrderBlock(
      ...
      children: [
        SizedBox(
          ...
          child: OutlinedButton(
            ...
            child: Text(
              VapeOrderFlowStrings.cancelOrder,
