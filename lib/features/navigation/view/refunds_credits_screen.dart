...
final _refundTxProvider = FutureProvider<List<WalletTransaction>>((ref) {
  final storage = ref.watch(storageServiceProvider);
  if (!storage.hasSession) return Future.value(const []);
  return ref.watch(walletRepositoryProvider).fetchTransactions(ledger: 'REFUND');
});

class RefundsCreditsScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final txAsync = ref.watch(_refundTxProvider);
          ...
          Expanded(
            child: RefreshIndicator(
              ...
              onRefresh: () async {
                ...
                ref.invalidate(_refundTxProvider);
                await Future.wait([
                  ref.read(walletSnapshotProvider.future),
                  ref.read(_refundTxProvider.future),
                ]);
              },
              child: ListView(
